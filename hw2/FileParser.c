...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  // If the input was bad and we get a -1, check if it is recoverable. If
  ...
  // Opened with -1, may be an unrecoverable error. If so, return NULL
  if (fd == -1) {
    fprintf(stderr,
              "Failed to open file");
    return NULL;
  ...
  // Loop until we've read all of the files contents
  while (left_to_read > 0) {
    ...
    // Remove number of chars read from what we have left to read
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  // Loops through the entire string and on each space separated word
  // within the string calls AddWordPosition to add it as well as its
  // position within the file to hashtable tab.
  while (cur_ptr[0] != '\0') {  // While the file contents string isn't ended
    if (!isalpha(cur_ptr[0])) {  // letter is not alphabetical
      // end the wordstart pointer by replacing the current
      // char witha '\0'. Reset the word start ptr to the next char.
      cur_ptr[0] = '\0';

      // If the word is in fact a word (and not just multiple
      // non-alphabetical characters in a row) then we do nothing
      if (strcmp(word_start, "")) {
        AddWordPosition(tab, word_start, word_start-content);
      }
      word_start = cur_ptr+1;
    } else {
      // sets every character to lower case where applicable
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    // Based on the inputted words size, allocate space for the wor
    // so that it doesn't get overrided in memory when we leave the function
    size_t wordsize = strlen(word)+1;
    char* newword = (char*) malloc(wordsize);
    Verify333(newword != NULL);
    strncpy(newword, word, wordsize);

    // Allocates new space for the word postion in memory before it is stored.
    // Then populates the word position with the malloced word and and
    // a new linkedlist containing its position in the file
    ...
    // Inserts the newly created word postion into our hash table
    // with the hash key we previously acquired
    HTKeyValue_t newkeyvalue;
    newkeyvalue.key = hash_key;
    newkeyvalue.value = (HTValue_t) wp;
    Verify333(!HashTable_Insert(tab, newkeyvalue, &kv));
  }
}
