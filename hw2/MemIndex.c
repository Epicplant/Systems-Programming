...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    //     mapping.
    wp->postings = HashTable_Allocate(50);
    ...
    Verify333(!HashTable_Insert(index, mi_kv, &unused));
  } else {
  ...
  Verify333(!HashTable_Insert(wp->postings, postings_kv, &unused));
}
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  // Allocate space for the ret_list and find the hash for the first word
  ret_list = LinkedList_Allocate();
  key = FNVHash64((unsigned char*) query[0], strlen(query[0]));

  // If the key doesn't have an index then we can't find
  // anything and we return null
  ...
  // Iterate through though the hash table, with us adding a
  // key whenever we encounter a new one.
  HTIterator* iter = HTIterator_Allocate(wp->postings);
  while (HTIterator_IsValid(iter)) {
    // If we encounter a new key then we enter the loop and add
    // it and its number of postings to
    // a search result
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      // Get the current node within our SearchResult linkedlist.
      // If we don't have this doc_id in the current list of
      // books found in the first word, then simply remove it
