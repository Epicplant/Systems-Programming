...
static void HandleDir(char* dir_path, DIR* d, DocTable** doc_table,
  ...
  // While there are still files in the directory continue
  dirent = readdir(d);
  for (i = 0; dirent != NULL; i++) {
    ...
    if (stat(entries[i].path_name, &st) == 0) {
      ...
      if (S_ISDIR(st.st_mode)) {
        ...
        // Handle the file and process it
...
static void HandleFile(char* file_path, DocTable** doc_table,
  ...
  // of the file.
  char* fileword = ReadFileToString(file_path, &file_len);
  tab = ParseIntoWordPositionsTable(fileword);
