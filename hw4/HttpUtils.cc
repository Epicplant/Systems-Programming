...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // Obtain the absolute path of the test_file. Return false
  // if there is an error obtaining it
  char actual_test_path[PATH_MAX+1];
  if (!realpath(test_file.c_str(), actual_test_path))
    return false;
  actual_test_path[strlen(actual_test_path)+1] = '\0';

  // Obtain the absolute path for the directory. Return false
  // if there is an error obtaining it
  char actual_dir_path[PATH_MAX+1];
  if (!realpath(root_dir.c_str(), actual_dir_path))
    return false;
  int position = strlen(actual_dir_path);
  actual_dir_path[position] = '/';
  actual_dir_path[position+1] = '\0';

  // Check to see if the absolute path of the dir exists within
  // the absolute path of the file
  size_t request_end = (string(actual_test_path)).find(actual_dir_path);
