...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  int32_t readlen;
  int32_t filesize = memidx_bytes + doctable_bytes;

  // Iterate through every byte in the file, to a new 512 byte
  // chunk every iteration, and then fold that byte
  // into the crc.
  while (filesize > 0) {
    // Read in 512 bytes from the file
    readlen = fread(&buf[0], sizeof(uint8_t), kBufSize, f);

    if (readlen == 0) {
      return kFailedWrite;
    }

    // Fold the 512 read bytes into crc
    for (int i = 0; i < readlen; i++) {
      crc.FoldByteIntoCRC(buf[i]);
    }

    // Decrease length of fileize by amount we just read
    filesize -= readlen;
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    // Write bucket record and if it fails then return -1
    if (WriteHTBucketRecord(f, record_pos,
        LinkedList_NumElements(ht->buckets[i]),
        bucket_pos) == kFailedWrite) {
      return kFailedWrite;
    }

    // Write actually buckets and if it false then return -1
    ssize_t linkedlist_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
    if (linkedlist_bytes == kFailedWrite) {
      return kFailedWrite;
    }

    if (linkedlist_bytes == 0 && (LinkedList_NumElements(ht->buckets[i]) > 0)) {
      return kFailedWrite;
    }

    // Increment position of record by the size of one record
    // and the positon of buckets by the amount of bytes placed there
    record_pos += sizeof(BucketRecord);
    bucket_pos += linkedlist_bytes;
  }
...
static int WriteHTBucketRecord(FILE* f, IndexFileOffset_t offset,
  ...
  BucketRecord br(num_elts, bucket_offset);
  br.ToDiskFormat();
  ...
  if (fwrite(&br, sizeof(BucketRecord), 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    // STEP 7.
    // fseek() to where the ElementPositionRecord should be written,
    ...
    // Receives the keyvalue pair from the hashtable bucket list.
    HTKeyValue_t* keyer;
    LLIterator_Get(it, reinterpret_cast<LLPayload_t*>(&keyer));

    // Creates an element position record
    ElementPositionRecord er(element_pos);
    er.ToDiskFormat();

    // Write the created record
    if (fwrite(&er, sizeof(ElementPositionRecord), 1, f) != 1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
    ...
    int bytes = fn(f, element_pos, keyer);
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  if (fwrite(kv->value, file_name_bytes, 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  // Create the DocIDElement header and then write t
  ...
  // Increment the offset forward
  offset += sizeof(DocIDElementHeader);
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    DocPositionOffset_t* p_offset =
    reinterpret_cast<DocPositionOffset_t*>(&payload);
    position = DocIDElementPosition(*p_offset);
    ...
    offset += sizeof(DocIDElementPosition);
  ...
  return sizeof(DocIDElementHeader)
         + (sizeof(DocIDElementPosition) * num_positions);
}
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  int16_t word_bytes = strlen(wp->word);
