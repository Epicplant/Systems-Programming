...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t& curr_element : elements) {
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      list<DocPositionOffset_t> positionlist;
      for (int i = 0; i < curr_header.num_positions; i++) {
        DocIDElementPosition DIEP;
        Verify333(fread(&DIEP, sizeof(DocIDElementPosition), 1, file_) == 1);

        DIEP.ToHostFormat();
        positionlist.push_back(DIEP.position);
      }

      // STEP 3.
      // Return the positions list through the output parameter,
      // and return true.
      *ret_val = positionlist;
      return true;
    }
  }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file.
    Verify333(fseek(file_,
              offset_ + sizeof(BucketListHeader) + (sizeof(BucketRecord) * i),
              SEEK_SET) == 0);
